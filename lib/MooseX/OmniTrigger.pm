...
{ package MooseX::OmniTrigger::MetaRole::Attribute; use namespace::autoclean;
    ...
    # FIRST, WE CAN'T WRAP Instance SETTERS, AS WE HAVE NO GOOD WAY OF KNOWING WHETHER THEY'RE BEING
    # CALLED FOR SET OR FOR SET-RAW OPS.

    # IDEALLY, TO KEEP OmniTrigger WORKING AS MOOSE IS DEVELOPED AND REFACTORED, WE WANT TO WRAP
    # "BASIC" SETTERS/CLEARERS -- METHODS IN Moose::Meta::Attribute THAT DO NOTHING MORE THAN
    # SET/CLEAR THE VALUE THROUGH THE INSTANCE INTERFACE AS DIRECTLY AS POSSIBLE AND THUS AREN'T
    # LIKELY TO CHANGE. AND WE WANT TO WRAP NO MORE SETTERS/CLEARERS THAN ABSOLUTELY NECESSARY,
    # WHILE STILL INTERCEPTING EVERY "NORMAL" (AND OMITTING EVERY RAW AND INTERNAL-SPECIAL-PURPOSE)
    # SET/CLEAR OPERATION.

    # WHAT WE'D LIKE IS FOR MOOSE TO GIVE US A PAIR OF BASIC SETTERS FOR HOOKING (ONE MUTABLE, ONE
    # IMMUTABLE) THROUGH WHICH *ALL* NON-RAW SET OPS MUST ULTIMATELY PASS (INCLUDING INIT-SET OPS).
    # IT'D BE SWEET IF THESE SETTERS HANDLED WEAKENING,TOO, WHICH ALWAYS OCCURS IMMEDIATELY AFTER
    # THE SET OP. (WE'D CONSIDER WEAKENING TO BE PART OF BASIC SETTING.)
    ...
    # CAN'T THINK OF A GOOD MOOSEY WAY TO HANG A METHOD MODIFIER ON Moose::Meta::Attribute'S
    # *SUPER'S* set_value WITHOUT CAUSING PROBLEMS. FORTUNATELY, IT LOOKS LIKE set_value IS THE ONLY
    # (MUTABLE NON-INIT) SETTER IN Attribute, AND IT'S ORGANIZED WITH TRIGGERS IN MIND, ANYWAY, WITH
    # THE TRIGGER OP BEING CONVENIENTLY THE LAST THING TO HAPPEN. WE CAN WRAP set_value AND FEEL
